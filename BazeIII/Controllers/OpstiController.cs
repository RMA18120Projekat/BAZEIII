...
namespace BazeIII.Controllers
{
    ...
    public class OpstiController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("OdzmiSmerOpstem/{Smer}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public IActionResult SmerOpstemOduzmi([FromRoute(Name = "Smer")] string Smer, [FromBody] string Opsti)
        {
            try
            {
                DTOManager.oduzmiSmerOpstem(Smer, Opsti);
                return Ok();
            }
            catch (Exception ex)
            {
