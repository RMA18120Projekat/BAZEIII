...
namespace BazeIII.Controllers
{
    ...
    public class PredmetController:ControllerBase
    {
        ...
        [HttpPost]
        [Route("OduzmiUcenikaPredmetu/{Jmbg}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public IActionResult OduzmiUcenikPredmetu([FromRoute(Name = "Jmbg")] long Jmbg, [FromBody] string Naziv)
        {
            try
            {
                DTOManager.oduzmiUcenikaPredmetu(Jmbg, Naziv);
                return Ok();
            }
            catch (Exception ex)
            {
