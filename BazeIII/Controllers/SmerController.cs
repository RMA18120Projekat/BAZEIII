...
namespace BazeIII.Controllers
{
    ...
    public class SmerController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("OduzmiOpstiSmeru/{NazivPredmeta}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public IActionResult OduzmiOpstiSmeru([FromRoute(Name = "NazivPredmeta")] string NazivPredmeta, [FromBody] string Smer)
        {
            try
            {
                DTOManager.oduzmiOpstiSmeru(NazivPredmeta, Smer);
                return Ok();
            }
            catch (Exception ex)
            {
