    ...
    public class UsmereniController : ControllerBase
    {
        ...
        public IActionResult AzurirajUsmereni([FromRoute(Name ="Smer")]string Smer,[FromBody] Skola.UsmereniAzur Usmereni,string Naziv)
        {
            try
            {
                ...
                UsmereniPregled u = DTOManager.vratiUsmereni(Naziv);
                u.Godina_studija = Usmereni.Godina_studija;
                SmerPregled s = DTOManager.vratiSmer(Smer);
                u.Smer = s;
                DTOManager.azurirajUsmereni(u);
                return Ok();
            }
            catch (Exception ex)
            {
